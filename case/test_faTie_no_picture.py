...
class Test_FaTie_No_Picture(unittest.TestCase):
    ...
    def test_TestFaTie(self):
        ...
        logger.debug(("获取header="+json.dumps(dict(res.headers))))
        logger.debug(("获取status_code="+str(res.status_code)))
        ret = res.json()
    ...
    def test_TestTieZiShow(self):
        ...
        cur_time = get_cur_time('%Y-%m-%d  %H:%M:%S')
        cookie_login = get_cookie()
        logger.debug('test_TestTieZiShow获取cookie='+json.dumps(cookie_login))
